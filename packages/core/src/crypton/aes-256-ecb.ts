...
export class AES256ECB {
  ...
  private keyExpansion(key: Buffer): void {
    ...
    for (let i = 8; i < 4 * (Nr + 1); i++) {
      ...
      if (i % Nk === 0) {
        this.rotWord(temp);
        this.subWord(temp);
        temp[0] ^= AES256ECB.rcon[Math.floor(i / Nk)];
        for (let t = 0; t < 4; t++) {
  ...
  private rotWord(word: Buffer): void {
    const t = word[0];
    word[0] = word[1];
    word[1] = word[2];
    word[2] = word[3];
  ...
  private subWord(word: Buffer): void {
    for (let i = 0; i < 4; i++) word[i] = AES256ECB.sBox[word[i]];
  }
  ...
  private subBytes(state: Buffer): void {
    for (let i = 0; i < 16; i++) state[i] = AES256ECB.sBox[state[i]];
  }

  private invSubBytes(state: Buffer): void {
    for (let i = 0; i < 16; i++) state[i] = AES256ECB.invSBox[state[i]];
  }
