...
describe('CryptoClient Extended', () => {
    ...
    test('encryptForServerSession with session', async () => {
        const ctx = createTestContext();
        const client = new CryptoClient(ctx, { mode: 'client' });
        await client.initialize();
        const server = new CryptoClient(ctx, { mode: 'server' });
        ...
        const enc = await server.encryptForServerSession(peerId, msg);
        const dec = await client.decryptForSession(peerId, enc, false);
        assert.ok(dec.data.equals(msg), 'server→client session roundtrip');
